...
class GradeManager:
    ...
    def get_student_average(self, student_name):
        ...
        """
        grades = [entry["grade"] for entry in self.students if entry["student_name"] == student_name]
    ...
    def get_subject_statistics(self, subject):
        ...
        subject_grades = [entry["grade"] for entry in self.students if entry["subject"] == subject]
    ...
    def get_top_students(self, n=3):
        ...
        print('student_totals:', student_totals)

        for entry in self.students:
            student_totals[entry["student_name"]].append(entry["grade"])

        averages = [
            (name, sum(grades) / len(grades))
            for name, grades in student_totals.items()
        ]

        sorted_averages = sorted(averages, key=lambda x: x[1], reverse=True)
        return sorted_averages[:n] if n <= len(sorted_averages) else sorted_averages
    ...
    def get_failing_students(self, passing_grade=60):
        ...
        for student in self.students:
            if student["grade"] < passing_grade:
                failing_Students.append((student["student_name"], student["grade"]))
